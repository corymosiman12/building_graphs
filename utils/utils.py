...
def find_equips(bldg_entities):
    equips = [r for r in bldg_entities if 'equip' in r.keys()]
    return equips
...
def find_str_in_navName(entities, find):
    matches = [e for e in entities if find.lower() in e['navName'].lower()]
    non_matches = [e for e in entities if not find.lower() in e['navName'].lower()]
    return (matches, non_matches)
